...
const Corp4 = ({ data }) => {
  ...
  const datas = data?.documents?.map((doc, index) => ({
    ...
    uploadDate: new Date(doc.uploadedAt || data?.updatedAt).toLocaleDateString(),
    action: (
    ...
    <div>
      ...
      {previewImage && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-white p-4 rounded shadow-xl max-w-[90%] max-h-[90%]">
            <img
              src={previewImage}
              alt="Preview"
