...
const BuyOrder = () => {
  ...
  const fetchBuyOrders = async () => {
    try {
      const response = await axiosInstance.get("/admin/getBuyOrders");
...
useEffect(() => {
  ...
  const renderRow = (row, index) => {
    return (
      ...
      >
        ...
        <td className="py-4 px-8">
          <div className="flex text-green-600 text-xl font-semibold items-center gap-2 ">
            {row.totalOrder}
          </div>
        </td>
        ...
        <button className="p-2 rounded py-5">
          ⁝
        </button>
        </td>
      </tr>
  ...
  return (
    <div className="flex  gap-4 w-full bg-slate-50 mt-4">
      <Table headers={headers} data={buyOrders} renderRow={renderRow} />
    </div>
  );
};
